"""Decode a bytearray into its string representation using the generation III character encoding"""
...
CHARACTER_MAP_JPN = {
    ...
    0xfa: '\0', 0xfb: '\0', 0xfc: '\0', 0xfd: '\0', 0xfe: '\n',
    0xff: '\0'
}


CHARACTER_MAP_NON_JPN = {
    # Instances of � are used to denote characters that can't be
    # easily represented using Unicode characters
...
def get_placeholder(codepoint: int, version: Version):
    """Returns a placeholder character for an unused codepoint

    Arguments:
        codepoint -- the codepoint to fetch a placeholder for
    ...
    if (
        ...
        # In R/S internationalisations, all unused codepoints
        # map to the corresponding characters in the Japanese character set
        return (
            CHARACTER_MAP_JPN[codepoint]
            if codepoint in CHARACTER_MAP_JPN
            else ' '
        )
    else:
        # All other versions print spaces as a placeholder
        return ' '


def build_character_map(version: Version, langauge: Language) -> dict[int, str]:
    ...
    """
    character_map = (
        CHARACTER_MAP_NON_JPN.copy() 
        if langauge is not Language.JPN
        else CHARACTER_MAP_JPN.copy()
        )
    if (
        version is Version.RUBY
        or version is Version.SAPPHIRE
        ...
        character_map[0xB0] = '‥'
    if langauge is Language.JPN and version is Version.FIRERED:
