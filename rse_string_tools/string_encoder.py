"""Encode strings into bytes using the generation III character encoding"""

from rse_string_tools.enums import Version, Language
from rse_string_tools.string_decoder import build_character_map
...
def build_character_map_r(
    ...
    # 0xF7-0xF9 are not control characters in R/S
    # so they will print normally as arrows.
    # However all gen III internationalisations also have arrows
    # in 0x79-0x7C, so this mapping only applies to Japanese versions
    if (
    ...
    if (
        ...
        # Build a reference character map for the used codepoints
