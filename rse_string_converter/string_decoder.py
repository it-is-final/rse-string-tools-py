...
from rse_string_converter.string_encoder import get_character_map
...
def get_character_map_r(version: Version, langauge: Language) -> dict[str, int]:
    character_map: dict[str, int]
    if langauge is Language.JPN:
        character_map = CHARACTER_MAP_R_JPN.copy()
        if (
        ...
        character_map = CHARACTER_MAP_R_NON_JPN.copy()
        ...
        if (
            ...
            japanese_map = get_character_map(version, Language.JPN)
            reference_map = get_character_map(version, langauge)
            unused_codepoints = (
                {x for x in range(0x0, (1 << 8))}
                .difference(set(reference_map.keys()))
                )
...
def decode_string(data: str, version: Version, language: Language):
    character_map = get_character_map_r(version, language)
