...
def get_character_map(version: Version, langauge: Language) -> dict[int, str]:
    character_map: dict[int, str]
    if (langauge is Language.JPN):
        character_map = CHARACTER_MAP_JPN.copy()
        if version is Version.FIRERED or Version.LEAFGREEN:
            character_map[0xB0] = "…"
    else:
        character_map = CHARACTER_MAP_NON_JPN.copy()
        if langauge is Language.FRA:
            character_map[0xB1] = "«"
            character_map[0xB2] = "»"
            if version is not Version.EMERALD:
                character_map.pop(0x64)
        else:
            character_map.pop(0x64)
        if langauge is Language.GER:
            character_map[0xB1] = "„"
            character_map[0xB2] = "“"
        if langauge is not Language.ITA:
            for i in range(0x5E, 0x64):
                character_map.pop(i)
        if version is Version.RUBY or Version.SAPPHIRE:
            character_map[0xB0] = "‥"
        if version is not Version.EMERALD:
    ...
    unused_characters = (
        ...
        .difference(character_map)
    )
    for i in unused_characters:
        character_map[i] = get_placeholder(i, version)
    return character_map


def encode_bytes(data: Iterable[int], version: Version, language: Language):
    character_map = get_character_map(version, language)
